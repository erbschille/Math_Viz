...
#violinplot version
ggplot(sensory_characteristics_table, aes(y = value, x = sensory_channel, fill = sensory_channel)) +
  ...
  theme_ipsum() +
  theme(legend.position = "none") +
  ylab("Response: Never (0) to Always (100)") +
  xlab("Sensory Modality")
```


# Figure 2

```{r "Cultivating Vividness", fig.asp=1.25}
math_survey <- math_survey %>% 
  mutate(session_vivid = factor(session_vivid, levels = c("Less vivid", "They stay consistent in their  vividness", "It just depends", "It depends on where I am in a reasoning process", "More vivid")),
         domain_vivid = factor(domain_vivid, levels = c("Less  vivid", "They stay consistent in their vividness", "It just depends", "More  vivid")))

plot_data <- math_survey %>%
  count(session_vivid) %>%
  complete(session_vivid, fill = list(n = 0))

ggplot(plot_data, aes(y = session_vivid, x = n)) +
  geom_col(fill = "#C3D7A4") +
  geom_text(
    aes(label = n),
    size = 6,
    hjust = -0.1
  ...
  coord_cartesian(xlim = c(0, max(plot_data$n) + 1))


ggplot(math_survey, aes(y = domain_vivid)) + 
  geom_bar(fill = "#4E84C4") + 
  theme_bw() +
    theme_bw() +
    geom_text(
    stat = 'count', 
    aes(label = ..count..), size = 6,
    color = "white",
    hjust = 1.2
  ) +
  labs(
    title = "As I become expert in a domain,\nvisualizations typically become...",
    x = "Count", 
    y = ""
  ) +
...
pref_characteristics_table <- survey_minus_zeros %>% 
  mutate(
    silent = 100 - noise_noisy_1
    , alone = 100 - people_company_1
    , `need particular surface`  = surface_need_1 
    , `hands idle` = 100 - hands_active_1
    , `dim light` = 100 - light_bright_1
    , `empty space` = 100 - space_full_1) %>% 
...
#violinplot version
ggplot(pref_characteristics_table,
       aes(x = preference, y = value, fill = preference)) +
  geom_violin(scale = "width", alpha = 0.6) +
  geom_dotplot(binaxis = "y",
               stackdir = "center",
               dotsize = 0.3) +
  ...
  xlab("Response: Must be the opposite (0) to Must be this way (100)") +
  ylab("Environmental Preferences")
```
...
#violinplot version
ggplot(control_table, aes(x = control_type, y = value, fill = control_type)) +
  geom_violin(scale = "width", alpha = 0.6) +
  geom_dotplot(binaxis = "y",
               stackdir = "center",
               dotsize = 0.5) +
...
ontology_table <- math_survey %>% 
  mutate(
    `Not real in any sense (0) or Real in some sense (100)` = object_real_4
    , `Don't physically exist (0) or Physically exist (100)` = Q50_4
    , `Build (0) or Discover (100)`  = build_see_1) %>% 
  select(ResponseId, `Not real in any sense (0) or Real in some sense (100)`:`Build (0) or Discover (100)`) %>% 
  pivot_longer(cols=c(`Not real in any sense (0) or Real in some sense (100)`:`Build (0) or Discover (100)`), names_to="question", values_to="value") %>%
  group_by(question) %>%
  mutate(mean_value = mean(value, na.rm = TRUE)) %>%
  ungroup()
...
#violinplot version
ggplot(ontology_table, aes(x = question, y = value, fill = question)) +
  geom_violin(scale = "width", alpha = 0.6) +
  geom_dotplot(binaxis = "y",
               stackdir = "center",
               dotsize = 0.5) +
