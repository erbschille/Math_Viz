...
```{r}
#same sensory information but as a ridgeline plot
...
note from ESH: something is going wrong here currently

```{r}
# gotta get rid of weird zeros (making a smaller, cleaner dataset)
...
pref_characteristics_table <- survey_minus_zeros %>% 
  mutate(
    silence = noise_noisy_1
    , alone = people_company_1
    , surface_doesnt_matter  = surface_need_1
    , hands_idle = hands_active_1
    , dim_light = light_bright_1
    , empty_space = space_full_1) %>% 
  select(ResponseId, silence:empty_space) %>% 
  pivot_longer(cols=c(silence:empty_space), names_to="preference", values_to="value") %>%
  group_by(preference) %>%
  mutate(mean_value = mean(value, na.rm = TRUE)) %>%
  ungroup()
...
```{r}
#object_real
 ggplot(math_survey, aes(x = object_real_4)) +
  #stat_density(geom="line", color = "blue", size = 1.2) +
  geom_rect(aes(xmin = t.test(math_survey$object_real_4)$conf.int[1], xmax = t.test(math_survey$object_real_4)$conf.int[2], ymin = -Inf, ymax = Inf), 
            alpha = 0.01, fill = "grey") +
   geom_vline(xintercept = mean(math_survey$object_real_4, na.rm = T), linetype = "dashed", color = "black", size = 0.5) +
  geom_histogram(color = "black", fill = "lightblue", alpha = 0.7) +
  scale_y_continuous(name = "Count", limits = c(0, 10)) +
  scale_x_continuous(limits=c(0, 100), breaks = c(0, 50, 100), labels = c("Not real in any sense", "Not sure", "Real in some sense"))+
  theme_bw(base_size=14) +
  labs(title = "To what extent do you think the impossible-to-build objects\nyou visualize are, in some sense, real?",
       x = "Responses on scale of 0 to 100") +
  theme(plot.title = element_text(hjust = 0.5, size = 15),
         plot.margin = margin(t = 20, r = 40, b = 20, l = 20))

#exist in physical world plot 
ggplot(math_survey, aes(x = Q50_4)) +
  #stat_density(geom="line", color = "blue", size = 1.2) +
  geom_rect(aes(xmin = t.test(math_survey$Q50_4)$conf.int[1], xmax = t.test(math_survey$Q50_4)$conf.int[2], ymin = -Inf, ymax = Inf), 
            alpha = 0.01, fill = "grey") +
   geom_vline(xintercept = mean(math_survey$Q50_4, na.rm = T), linetype = "dashed", color = "black", size = 0.5) +
  geom_histogram(color = "black", fill = "lightblue", alpha = 0.7) +
  scale_y_continuous(name = "Count", limits = c(0, 10)) +
  scale_x_continuous(limits=c(0, 100), breaks = c(0, 50, 100), labels = c("Don't exist in any sense", "Not sure", "Exist in some sense"))+
  theme_bw(base_size=14) +
  labs(title = "To what extent do you think the impossible-to-build objects\nyou visualize exist in the physical world, in some sense?",
       x = "Responses on scale of 0 to 100") +
  theme(plot.title = element_text(hjust = 0.5, size = 15), 
         plot.margin = margin(t = 20, r = 40, b = 20, l = 20))

# build_see plot
ggplot(math_survey, aes(x = build_see_1)) +
  #stat_density(geom="line", color = "blue", size = 1.2) +
   geom_rect(aes(xmin = t.test(math_survey$build_see_1)$conf.int[1], xmax = t.test(math_survey$build_see_1)$conf.int[2], ymin = -Inf, ymax = Inf), 
            alpha = 0.01, fill = "grey") +
   geom_vline(xintercept = mean(math_survey$build_see_1, na.rm = T), linetype = "dashed", color = "black", size = 0.5) +
  geom_histogram(color = "black", fill = "lightblue", alpha = 0.5) +
  scale_y_continuous(name = "Count", limits = c(0, 10)) +
  scale_x_continuous(limits=c(0, 100), breaks = c(0, 50, 100), labels = c("Building", "", "Discovering"))+
  theme_bw(base_size=14) +
  labs(title = "When you visualize, does it feel more like you are building\nthe objects or like you are seeing things that are 'out there'?",
       x = "Responses on scale of 0 to 100") +
  theme(plot.title = element_text(hjust = 0.5, size = 15),
        plot.margin = margin(t = 20, r = 40, b = 20, l = 20))
...
control_table2 <- math_survey %>% 
  ...
  dplyr::select(ResponseId, , "Any causal control?":"Ever annoyed by them?") %>% 
  pivot_longer(cols=c("Any causal control?":"Ever annoyed by them?"), names_to="question", values_to="response") %>%
  drop_na(response) %>%
...
corrplot(cor(msr %>% select(hands_active, noise_noisy, space_full, light_bright, people_company, visualization_proportion), use="pairwise.complete.obs"), order='hclust', method = "number")

corrplot(cor(msr %>% select(zoom, tactile, color, dynamic, domain_increasing_vividness, session_increasing_vividness, visualization_proportion, shape, auditory, olfactory, relative_location), use="pairwise.complete.obs"), order='hclust', method = "number")
...
#just control items 
corrplot(cor(math_survey %>% 
               select(causal, constraint, appear, fade, stay_present, stop_seeing, annoyed) %>%
                mutate(across(c(causal, constraint, appear, fade, stay_present, stop_seeing, annoyed), 
                ~ ifelse(. == "Yes", 1, ifelse(. == "No", 0, NA)))), 
             use="pairwise.complete.obs"), order='hclust', method = "number")
...
```{r}
math_survey_rename$domain_coding <- "not coded"
math_survey_rename$domain_coding[math_survey_rename$domains %>% grep("[t|T]opology", .)] <- "Topology"
math_survey_rename$domain_coding[math_survey_rename$domains %>% grep("[t|T]opological", .)] <- "Topology"
math_survey_rename$domain_coding[math_survey_rename$domains %>% grep("[a|A]lgebraic [G|g]eometry", .)] <- "Algebraic Geometry"
math_survey_rename$domain_coding[math_survey_rename$domains %>% grep("[a|A]lg [G|g]eom", .)] <- "Algebraic Geometry"
math_survey_rename$domain_coding[math_survey_rename$domains %>% grep("[P|p]robability", .)] <- "Probability"
math_survey_rename$domain_coding[math_survey_rename$domains %>% grep("[N|n]umber [T|t][h|g]eory", .)] <- "Number Theory"
math_survey_rename %>% summarize(ct=n(), .by=domain_coding) %>% arrange(-ct)
